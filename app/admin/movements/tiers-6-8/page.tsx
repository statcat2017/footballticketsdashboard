...
export default async function Tiers6to8Page(props: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const targetDivisions = await getDivisionsInTierRange(db, 5, 9);
    ...
    <main style={{ maxWidth: "64rem", margin: "0 auto", padding: "0 1rem 5rem", fontFamily: "system-ui, sans-serif" }}>
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1.25rem 0", borderBottom: "1px solid #dce3e2", marginBottom: "1.5rem" }}>
        <div>
          ...
          <h1 style={{ margin: "0.25rem 0 0", fontSize: "1.5rem" }}>Tiers 6–8: National League North/South → Tier 8</h1>
          <p style={{ margin: "0.25rem 0 0", color: "#6f7e7a", fontSize: "14px" }}>{view.totalSlots} slots &middot; {view.totalFilled} filled &middot; {view.affectedDivisions} affected divisions</p>
        </div>
      ...
      <section style={{ border: "1px solid #dce3e2", borderRadius: "8px", padding: "1rem", marginBottom: "1.5rem", background: "#f5f7f7" }}>
        ...
        <form method="post" action="/api/admin/movements/slots" style={{ display: "flex", gap: "0.75rem", alignItems: "flex-end", flexWrap: "wrap", marginTop: "0.75rem" }}>
          ...
          <div>
            ...
            <input type="number" name="count" min="1" max="50" defaultValue="3" style={{ padding: "0.4rem 0.6rem", border: "1px solid #dce3e2", borderRadius: "6px", fontSize: "13px", width: "60px" }} />
          </div>
