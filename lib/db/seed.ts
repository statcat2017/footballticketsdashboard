...
export function seedDatabase(db: SqliteDatabase): void {
  const count = db.prepare("SELECT COUNT(*) as count FROM competitions").get() as { count: number };

  if (count.count > 0) {
    return;
  }

  const insertCompetition = db.prepare("INSERT INTO competitions (code, name, tier) VALUES (?, ?, ?)");
