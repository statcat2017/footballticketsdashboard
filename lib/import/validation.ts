...
import type { ImportBatchRow, MatchResult } from "./types.ts";
import {
...
export async function validateImportBatch(
  ...
  for (const row of rows) {
    ...
    await updateBatchRowOutcome(db, row.id, {
...
export async function validateRow(
  ...
  const home = await resolveParticipant(db, row.homeParticipantRaw, row.homeIsOneOff, row.competitionResolvedCode ?? undefined, "home");
  warnings.push(...home.warnings);
  if (home.isBlocked) hasBlocker = true;

  const away = await resolveParticipant(db, row.awayParticipantRaw, row.awayIsOneOff, row.competitionResolvedCode ?? undefined, "away");
  warnings.push(...away.warnings);
  if (away.isBlocked) hasBlocker = true;

  const comp = await resolveCompetition(db, row.competitionRaw, home.clubId);
  warnings.push(...comp.warnings);
  if (comp.isBlocked) hasBlocker = true;
  ...
  if (dateStr) {
    ...
    if (!parsed) {
      ...
      dateStr = parsed.date;
      if (parsed.time) kickoffTime = parsed.time;
    }
  } else {
  ...
  if (row.status) {
    const valid = ["scheduled", "postponed", "cancelled", "finished", "unknown"];
    if (!valid.includes(row.status)) {
      warnings.push({ field: "status", message: `Invalid status: ${row.status}. Defaulting to scheduled.` });
    }
  }
  ...
  if (hasBlocker) {
    return {
      ...
      competitionResolvedCode: comp.code,
