...
describe("admin route handlers", () => {
  ...
  it("creates a venue from POST /api/admin/venues form data", async () => {
    ...
    expect(response.status).toBe(303);
    expect(response.headers.get("location")).toMatch(/^http:\/\/localhost\/admin\/venues\/\d+$/);
    ...
    }>("SELECT id, name, postcode, latitude, longitude, is_approximate, coordinate_precision FROM venues WHERE name = ?", ["New Stadium"]);
